...
namespace Flexa\MailBridge\Reports;
...
final class Digest {
	...
	public static function send( int $days, string $period_label ): array {
		...
		$subject = sprintf(
			...
			__( '[%1$s] Your %2$s email report', 'flexa-mailbridge' ),
	...
	public static function render( array $summary, string $period_label ): string {
		...
		$cards = [
			__( 'Sent', 'flexa-mailbridge' )       => number_format_i18n( (int) ( $totals['sent'] ?? 0 ) ),
			__( 'Failed', 'flexa-mailbridge' )     => number_format_i18n( (int) ( $totals['failed'] ?? 0 ) ),
			__( 'Opens', 'flexa-mailbridge' )      => number_format_i18n( (int) ( $totals['opens'] ?? 0 ) ),
			__( 'Clicks', 'flexa-mailbridge' )     => number_format_i18n( (int) ( $totals['clicks'] ?? 0 ) ),
			__( 'Open rate', 'flexa-mailbridge' )  => ( (float) ( $totals['open_rate'] ?? 0 ) ) . '%',
			__( 'Click rate', 'flexa-mailbridge' ) => ( (float) ( $totals['click_rate'] ?? 0 ) ) . '%',
		];
		...
		$header = sprintf(
			...
			esc_html__( 'Your %1$s email report (%2$s – %3$s)', 'flexa-mailbridge' ),
		...
		return sprintf(
			...
			$rows,
			'' !== $links
				? '<h3 style="color:#111;font-size:15px;margin-top:20px;">' . esc_html__( 'Top links', 'flexa-mailbridge' ) . '</h3><ul style="padding-left:18px;margin:0;">' . $links . '</ul>'
				: ''
		);
	}
}
