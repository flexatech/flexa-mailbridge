 ...
 * match the ones {@see \Flexa\MailBridge\Setup\Deactivator} clears, so deactivation
...
final class Scheduler {
	...
	public const WEEKLY            = 'flexa_mailbridge_report_weekly';
	public const MONTHLY           = 'flexa_mailbridge_report_monthly';
	private const MONTHLY_SCHEDULE = 'flexa_mailbridge_monthly';
	...
	public function add_monthly_schedule( array $schedules ): array {
		$schedules[ self::MONTHLY_SCHEDULE ] = [
			...
			'display'  => __( 'Once Monthly (Flexa MailBridge)', 'flexa-mailbridge' ),
		];
	...
	public function run_weekly(): void {
		if ( (bool) Settings::get( 'enable_weekly_report' ) ) {
			Digest::send( 7, __( 'weekly', 'flexa-mailbridge' ) );
		}
	}

	public function run_monthly(): void {
		if ( (bool) Settings::get( 'enable_monthly_report' ) ) {
			Digest::send( 30, __( 'monthly', 'flexa-mailbridge' ) );
		}
	}
}
