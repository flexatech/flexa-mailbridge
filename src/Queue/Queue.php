...
namespace Flexa\MailBridge\Queue;

use Flexa\MailBridge\Concerns\HasInstance;
use Flexa\MailBridge\Domain\EmailLog;
use Flexa\MailBridge\Domain\EmailLogRepository;
use Flexa\MailBridge\Mailer\MailerManager;
use Flexa\MailBridge\Mailer\PhpMailerBridge;
use Flexa\MailBridge\Mailer\Result;
...
final class Queue {
	...
	public const RUN_HOOK  = 'flexa_mailbridge_queue_run';
	public const TICK_HOOK = 'flexa_mailbridge_queue_tick';

	/** Action Scheduler group so our actions are easy to find and purge. */
	private const AS_GROUP = 'flexa-mailbridge';

	/** Recurring safety-net interval that catches any run a precise event missed. */
	private const TICK_SCHEDULE = 'flexa_mailbridge_fifteen_minutes';

	/** Short lock so overlapping worker runs do not thrash the same batch. */
	private const LOCK = 'flexa_mailbridge_queue_lock';
	...
	public function register(): void {
		...
		add_action( 'flexa_mailbridge.settings.updated', [ $this, 'sync' ] );
	}
	...
	public function enqueue( PhpMailerBridge $php, int $log_id, bool $from_sync_retry ): int {
		 ...
		 * @param bool $dedupe
		 */
		if ( ! $from_sync_retry && (bool) apply_filters( 'flexa_mailbridge.queue.dedupe', true ) && $this->repo()->active_key_exists( $key ) ) {
	...
	public function run_worker(): void {
		...
		try {
			...
			$claim = substr( md5( uniqid( 'flexa_mailbridge_', true ) ), 0, 32 );
	...
	private function process_item( QueueItem $item ): void {
		...
		if ( $result->ok ) {
			 ...
			 */
			do_action( 'flexa_mailbridge.queue.sent', $item, $result->to_meta() );
			return;
		}
		...
		if ( $retryable && $attempts < $item->max_attempts ) {
			 ...
			 * @param string    $error
			 */
			do_action( 'flexa_mailbridge.queue.retry_scheduled', $item, $error );
			return;
		}
		 ...
		 * @param string    $error
		 */
		do_action( 'flexa_mailbridge.queue.failed', $item, $error );
	}
	...
	public function add_schedule( array $schedules ): array {
		$schedules[ self::TICK_SCHEDULE ] = [
			...
			'display'  => __( 'Every Fifteen Minutes (Flexa MailBridge)', 'flexa-mailbridge' ),
		];
	...
	private function uses_action_scheduler(): bool {
		 ...
		 */
		return (bool) apply_filters( 'flexa_mailbridge.queue.use_action_scheduler', $available );
	}
	...
	private function batch_size(): int {
		 ...
		 * @param int $size
		 */
		return max( 1, min( 100, (int) apply_filters( 'flexa_mailbridge.queue.batch_size', 20 ) ) );
	}
	...
	private function time_budget(): float {
		 ...
		 * @param int $seconds
		 */
		return (float) max( 5, min( 120, (int) apply_filters( 'flexa_mailbridge.queue.time_budget', 20 ) ) );
	}
	...
	private function backoff( int $attempts ): int {
		 ...
		 * @param int $attempts
		 */
		return (int) apply_filters( 'flexa_mailbridge.queue.backoff', $steps[ $index ], $attempts );
	}
