 ...
 * passwords and API keys — before they land in the `flexa_mailbridge_settings` option.
 *
 ...
 * defines FLEXA_MAILBRIDGE_ENCRYPTION_KEY; either way it never ships in the DB, so a
...
final class Encryption {
	...
	private static function key(): string {
		if ( defined( 'FLEXA_MAILBRIDGE_ENCRYPTION_KEY' ) && is_string( FLEXA_MAILBRIDGE_ENCRYPTION_KEY ) && '' !== FLEXA_MAILBRIDGE_ENCRYPTION_KEY ) {
			return hash( 'sha256', FLEXA_MAILBRIDGE_ENCRYPTION_KEY, true );
		}

		$salt = function_exists( 'wp_salt' ) ? wp_salt( 'secure_auth' ) : 'flexa-mailbridge-insecure-fallback';

		// 32 raw bytes for AES-256.
		return hash( 'sha256', 'flexa-mailbridge|' . $salt, true );
	}
}
