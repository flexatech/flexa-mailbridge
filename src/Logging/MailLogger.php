...
namespace Flexa\MailBridge\Logging;
...
/**
 * Turns the mailer lifecycle hooks fired by {@see \Flexa\MailBridge\Mailer\MailerManager}
...
final class MailLogger {
	...
	public function register(): void {
		add_action( 'flexa_mailbridge.mail.before_send', [ $this, 'on_before_send' ], 10, 2 );
		add_action( 'flexa_mailbridge.mail.failed', [ $this, 'on_failed' ], 10, 4 );
		add_action( 'flexa_mailbridge.mail.sent', [ $this, 'on_sent' ], 10, 3 );
		// Fires when the opt-in queue takes ownership of the in-flight message (DL4).
		add_action( 'flexa_mailbridge.mail.queued', [ $this, 'on_queued' ], 10, 2 );
	...
	public function on_before_send( $php, $slug ): void {
		...
		if ( (int) $snapshot['id'] > 0 ) {
			 ...
			 */
			do_action( 'flexa_mailbridge.log.created', (int) $snapshot['id'], $php );
		}
	}
	...
	public function on_queued( $php, $status ): void {
		...
		if ( EmailLog::STATUS_RETRYING === $status && [] !== $this->errors ) {
			...
			$reason = __( 'Skipped: an identical message was already queued.', 'flexa-mailbridge' );
		}
	...
	public function on_wp_mail_failed( $error ): void {
		...
		$reason = [] !== $this->errors
			? implode( ' | ', $this->errors )
			: __( 'The message could not be sent.', 'flexa-mailbridge' );
