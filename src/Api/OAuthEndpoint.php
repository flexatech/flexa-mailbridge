...
use Flexa\MailBridge\Mailer\Contracts\UsesOAuth;
use Flexa\MailBridge\Mailer\MailerRegistry;
use Flexa\MailBridge\OAuth\StateSigner;
use Flexa\MailBridge\OAuth\TokenStore;
use WP_Error;
use WP_REST_Request;
...
final class OAuthEndpoint extends Endpoint {
	...
	public function callback_permission( WP_REST_Request $request ): bool|WP_Error {
		...
		if ( '' === $state || ! StateSigner::verify( $state, $slug ) ) {
			return new WP_Error(
				'flexa_mailbridge_oauth_state',
				__( 'The OAuth state could not be verified.', 'flexa-mailbridge' ),
				[ 'status' => 403 ]
			);
		}
	...
	private function unknown_provider( string $slug ): WP_Error {
		return new WP_Error(
			'flexa_mailbridge_unknown_provider',
			/* translators: %s: mailer slug. */
			sprintf( __( '"%s" is not an OAuth mailer.', 'flexa-mailbridge' ), $slug ),
			[ 'status' => 404 ]
		);
	}
