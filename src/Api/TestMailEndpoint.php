...
/**
 * POST /wp-json/flexa-mailbridge/v1/test-mail — send a test email through the full
	...
	public function send_test( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( '' === $to || ! is_email( $to ) ) {
			return new WP_REST_Response(
				[
					'ok'    => false,
					'error' => __( 'A valid recipient email address is required.', 'flexa-mailbridge' ),
				],
				400
			);
		}
		...
		if ( '' === $subject ) {
			$subject = sprintf(
				...
				__( 'Flexa MailBridge test email from %s', 'flexa-mailbridge' ),
		...
		$body    = $is_html
			? '<p>' . esc_html__( 'This is a Flexa MailBridge test email. If you received it, your mailer is configured correctly.', 'flexa-mailbridge' ) . '</p>'
			: __( 'This is a Flexa MailBridge test email. If you received it, your mailer is configured correctly.', 'flexa-mailbridge' );
		...
		add_filter( 'flexa_mailbridge.mail.bypass_queue', '__return_true' );
		$sent = wp_mail( $to, $subject, $body, $headers );
		remove_filter( 'flexa_mailbridge.mail.bypass_queue', '__return_true' );
		...
		return new WP_REST_Response(
			[
				'ok'    => false,
				'error' => '' !== $captured ? $captured : __( 'The test email could not be sent.', 'flexa-mailbridge' ),
			]
		);
	}
}
