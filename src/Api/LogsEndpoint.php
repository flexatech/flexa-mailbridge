...
use Flexa\MailBridge\Diagnostics\Diagnosis;
use Flexa\MailBridge\Diagnostics\Diagnostics;
use Flexa\MailBridge\Domain\ClickEventRepository;
use Flexa\MailBridge\Domain\EmailLog;
use Flexa\MailBridge\Domain\EmailLogRepository;
use Flexa\MailBridge\Domain\OpenEventRepository;
use WP_REST_Request;
...
final class LogsEndpoint extends Endpoint {
	...
	public function get_log( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( ! $log instanceof EmailLog ) {
			return new WP_REST_Response(
				[ 'error' => __( 'Log entry not found.', 'flexa-mailbridge' ) ],
				404
			);
		}
	...
	public function export_logs( WP_REST_Request $request ): WP_REST_Response {
		...
		header( 'Content-Disposition: attachment; filename="flexa-mailbridge-logs-' . gmdate( 'Ymd-His' ) . '.csv"' );
