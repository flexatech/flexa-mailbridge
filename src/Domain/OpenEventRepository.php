...
namespace Flexa\MailBridge\Domain;
...
/**
 * Reads and writes for `flexa_mailbridge_open_events`. One row per (log) accumulates
 * an open count; {@see record()} is the upsert the tracking pixel (WP6) calls.
 */
final class OpenEventRepository {
	private const TABLE = 'flexa_mailbridge_open_events';
	...
	public function stats_in_range( string $from, string $to ): array {
		...
		$open = $this->table();
		$logs = $wpdb->prefix . 'flexa_mailbridge_email_logs';
