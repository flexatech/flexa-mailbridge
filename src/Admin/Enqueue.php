 ...
 * page, then publishes the `flexaMailBridge` global the React app boots from.
 */
final class Enqueue {
	use HasInstance;

	private const HANDLE = 'flexa-mailbridge-admin';
	...
	public function enqueue_admin( string $hook_suffix ): void {
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations(
				self::HANDLE,
				FLEXA_MAILBRIDGE_TEXT_DOMAIN,
				FLEXA_MAILBRIDGE_PATH . 'i18n/languages'
			);
		}
		...
		wp_localize_script(
			...
			'flexaMailBridge',
			[
				'restUrl'           => esc_url_raw( rest_url( FLEXA_MAILBRIDGE_REST_NAMESPACE . '/' ) ),
				'restNonce'         => wp_create_nonce( 'wp_rest' ),
				'namespace'         => FLEXA_MAILBRIDGE_REST_NAMESPACE,
				'version'           => FLEXA_MAILBRIDGE_VERSION,
				'pluginUrl'         => esc_url_raw( FLEXA_MAILBRIDGE_URL ),
	...
	private function enqueue_prod( string $handle, string $entry ): void {
		$manifest_path = FLEXA_MAILBRIDGE_PATH . 'assets/dist/.vite/manifest.json';
		if ( ! is_readable( $manifest_path ) ) {
			$manifest_path = FLEXA_MAILBRIDGE_PATH . 'assets/dist/manifest.json';
		}
		if ( ! is_readable( $manifest_path ) ) {
			return;
		}
		...
		$item     = $manifest[ $entry ];
		$dist_url = FLEXA_MAILBRIDGE_URL . 'assets/dist/';

		foreach ( $this->collect_manifest_css( $manifest, $entry ) as $i => $css_file ) {
			wp_enqueue_style( $handle . '-css-' . $i, $dist_url . $css_file, [], FLEXA_MAILBRIDGE_VERSION );
		}
