 ...
 * offering to migrate it into Flexa MailBridge. Dismissible; the choice is stored so
 * the notice never nags again.
 */
final class ImportNotice {
	use HasInstance;

	private const DISMISSED_OPTION = 'flexa_mailbridge_import_notice_dismissed';
	private const DISMISS_ACTION   = 'flexa_mailbridge_dismiss_import';
	...
	public function render(): void {
		...
		printf(
			...
			esc_html__( 'Flexa MailBridge: import your existing configuration', 'flexa-mailbridge' ),
			esc_html(
				sprintf(
					/* translators: %s: comma-separated list of detected plugin names. */
					__( 'We detected settings from: %s. You can import them (and their email logs) into Flexa MailBridge.', 'flexa-mailbridge' ),
					implode( ', ', $labels )
				)
			),
			esc_url( $page_url ),
			esc_html__( 'Go to Import', 'flexa-mailbridge' ),
			esc_url( $dismiss_url ),
			esc_html__( 'Dismiss', 'flexa-mailbridge' )
		);
	}
}
