...
final class TransportCheck implements HealthCheck {
	...
	public function label(): string {
		return __( 'Provider connectivity', 'flexa-mailbridge' );
	}
		...
		if ( 'mail' === $slug ) {
			return [
				$this->row(
					'reachability',
					CheckResult::NA,
					__( 'PHP mail() hands off to the local mail server, so there is no SMTP or API endpoint to test here.', 'flexa-mailbridge' ),
					'',
					[ 'mailer' => $slug ]
				),
			];
	...
	private function smtp_result(): CheckResult {
		...
		if ( '' === $host || $port <= 0 ) {
			return $this->row(
				'reachability',
				CheckResult::ERROR,
				__( 'No SMTP host and port are configured.', 'flexa-mailbridge' ),
				__( 'Enter the SMTP host and port on the settings screen.', 'flexa-mailbridge' ),
				[ 'mailer' => 'smtp' ]
			);
		}
		...
		if ( ! function_exists( 'fsockopen' ) ) {
			return $this->row(
				...
				__( 'The server has disabled the socket functions needed to test the connection.', 'flexa-mailbridge' ),
				'',
				[
					'host' => $host,
					'port' => $port,
		...
		if ( false === $socket ) {
			return $this->row(
				...
				sprintf( __( 'Could not connect to %1$s on port %2$d.', 'flexa-mailbridge' ), $host, $port ),
				__( 'Check the host and port, and confirm your server allows outbound connections on that port.', 'flexa-mailbridge' ),
				[
					'host'  => $host,
					'port'  => $port,
					'errno' => $errno,
		...
		return $this->row(
			...
			CheckResult::PASS,
			/* translators: 1: host, 2: port. */
			sprintf( __( 'Connected to %1$s on port %2$d.', 'flexa-mailbridge' ), $host, $port ),
			'',
			[
				'host' => $host,
				'port' => $port,
	...
	private function api_result( string $slug ): CheckResult {
		...
		if ( ! $mailer->is_configured() ) {
			return $this->row(
				...
				CheckResult::ERROR,
				/* translators: %s: mailer slug. */
				sprintf( __( 'The "%s" mailer is missing required credentials.', 'flexa-mailbridge' ), $slug ),
				__( 'Complete the credentials for this mailer, then send a test email to confirm delivery.', 'flexa-mailbridge' ),
				[ 'mailer' => $slug ]
			);
		}
		...
		return $this->row(
			...
			CheckResult::PASS,
			/* translators: %s: mailer slug. */
			sprintf( __( 'The "%s" mailer has credentials configured. Use "Send test email" to confirm live delivery.', 'flexa-mailbridge' ), $slug ),
			'',
			[ 'mailer' => $slug ]
		);
	}
