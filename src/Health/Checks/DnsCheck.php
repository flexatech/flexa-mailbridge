...
final class DnsCheck implements HealthCheck {
	...
	public function label(): string {
		return __( 'Sending domain DNS', 'flexa-mailbridge' );
	}
	...
	public function run(): array {
		...
		if ( '' === $domain ) {
			return [
				$this->row( 'spf', CheckResult::NA, __( 'No public sending domain to check.', 'flexa-mailbridge' ) ),
			];
		}

		if ( ! function_exists( 'dns_get_record' ) ) {
			return [
				$this->row( 'spf', CheckResult::NOT_CHECKED, __( 'DNS lookups are disabled on this server.', 'flexa-mailbridge' ), '', [ 'domain' => $domain ] ),
			];
		}
	...
	private function spf_result( string $domain ): CheckResult {
		...
		foreach ( $records as $txt ) {
			if ( str_starts_with( strtolower( $txt ), 'v=spf1' ) ) {
				return $this->row(
					'spf',
					CheckResult::PASS,
					__( 'An SPF record is published for the sending domain.', 'flexa-mailbridge' ),
					'',
					[ 'domain' => $domain ]
				);
			}
		...
		return $this->row(
			'spf',
			CheckResult::WARN,
			__( 'No SPF record was found for the sending domain. Messages are more likely to be marked as spam.', 'flexa-mailbridge' ),
			__( 'Publish an SPF (TXT) record that authorises your provider to send for this domain.', 'flexa-mailbridge' ),
			[ 'domain' => $domain ]
		);
	}
	...
	private function dmarc_result( string $domain ): CheckResult {
		...
		foreach ( $records as $txt ) {
			if ( str_starts_with( strtolower( $txt ), 'v=dmarc1' ) ) {
				return $this->row(
					'dmarc',
					CheckResult::PASS,
					__( 'A DMARC record is published for the sending domain.', 'flexa-mailbridge' ),
					'',
					[ 'domain' => $domain ]
				);
			}
		...
		return $this->row(
			'dmarc',
			CheckResult::WARN,
			__( 'No DMARC record was found for the sending domain.', 'flexa-mailbridge' ),
			__( 'Publish a DMARC (TXT) record at _dmarc.yourdomain to tell receivers how to handle unauthenticated mail.', 'flexa-mailbridge' ),
			[ 'domain' => $domain ]
		);
	}
	...
	private function dkim_result( string $domain ): CheckResult {
		return $this->row(
			...
			__( 'DKIM uses a provider-specific selector that cannot be detected automatically.', 'flexa-mailbridge' ),
			__( 'Confirm DKIM is set up and passing in your email provider dashboard.', 'flexa-mailbridge' ),
			[ 'domain' => $domain ]
		);
	}
	...
	private function row( string $id, string $status, string $message, string $remediation = '', array $context = [] ): CheckResult {
		$labels = [
			'spf'   => __( 'SPF record', 'flexa-mailbridge' ),
			'dmarc' => __( 'DMARC record', 'flexa-mailbridge' ),
			'dkim'  => __( 'DKIM', 'flexa-mailbridge' ),
		];
