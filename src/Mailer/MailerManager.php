...
namespace Flexa\MailBridge\Mailer;

use Flexa\MailBridge\Concerns\HasInstance;
use Flexa\MailBridge\Diagnostics\Diagnostics;
use Flexa\MailBridge\Domain\EmailLog;
use Flexa\MailBridge\Logging\MailLogger;
use Flexa\MailBridge\Mailer\Contracts\PhpMailerConfigurator;
use Flexa\MailBridge\Mailer\Contracts\MailerInterface;
use Flexa\MailBridge\Queue\Queue;
...
final class MailerManager {
	...
	public function dispatch( PhpMailerBridge $php ): bool {
		if ( (bool) Settings::get( 'disable_delivery' ) ) {
			 ...
			 * @param string          $slug
			 */
			do_action( 'flexa_mailbridge.mail.before_send', $php, 'disabled' );
			do_action( 'flexa_mailbridge.mail.sent', $php, 'disabled', [ 'disabled' => true ] );
		...
		if ( $this->should_queue() ) {
			do_action( 'flexa_mailbridge.mail.before_send', $php, $this->primary_slug() );

			$log_id = MailLogger::instance()->current_log_id();
			$code   = Queue::instance()->enqueue( $php, $log_id, false );

			if ( Queue::ENQUEUE_OK === $code ) {
				do_action( 'flexa_mailbridge.mail.queued', $php, EmailLog::STATUS_QUEUED );
			...
			if ( Queue::ENQUEUE_DUPLICATE === $code ) {
				do_action( 'flexa_mailbridge.mail.queued', $php, EmailLog::STATUS_CANCELLED );
		...
		foreach ( $this->mailer_chain() as $slug ) {
			...
			if ( $result->ok ) {
				 ...
				 */
				do_action( 'flexa_mailbridge.mail.sent', $php, $slug, $result->to_meta() );
			 ...
			 * @param array<string, mixed> $meta
			 */
			do_action( 'flexa_mailbridge.mail.failed', $php, $slug, $last_error, $result->to_meta() );
		}
		...
		if ( $this->should_retry( $last_result ) ) {
			...
			if ( Queue::ENQUEUE_OK === Queue::instance()->enqueue( $php, $log_id, true ) ) {
				do_action( 'flexa_mailbridge.mail.queued', $php, EmailLog::STATUS_RETRYING );
				Queue::instance()->schedule_worker();
		...
		throw new Exception( esc_html( '' !== $last_error ? $last_error : 'flexa-mailbridge: no mailer could send the message' ) );
	}
	...
	public function deliver( PhpMailerBridge $php ): Result {
		...
		return $last ?? Result::error( 'flexa-mailbridge: no mailer could send the message' );
	}
	...
	private function should_queue(): bool {
		 ...
		 * @param bool $bypass
		 */
		if ( (bool) apply_filters( 'flexa_mailbridge.mail.bypass_queue', false ) ) {
			return false;
		}
	...
	private function should_retry( ?Result $result ): bool {
		 ...
		 * @param Result      $result
		 */
		return (bool) apply_filters( 'flexa_mailbridge.queue.should_retry', true, $result->error_category, $result );
	}
