# Flexa MailBridge — Implementation Plan (phased, core-first)

> Kế hoạch xây dựng `flexa-mailbridge` — plugin WP Mail SMTP + Email Logs + Tracking +
...
| Token | Giá trị |
|---|---|
| `<plugin-slug>` / text-domain | `flexa-mailbridge` |
| PHP namespace | `Flexa\MailBridge\` → `src/` (Pro: `Flexa\MailBridgePro\` → `src-pro/`) |
| `<CONST>` prefix | `FLEXA_MAILBRIDGE_` — `_VERSION/_FILE/_PATH/_URL/_BASENAME/_REST_NAMESPACE/_TEXT_DOMAIN` |
| REST namespace | `flexa-mailbridge/v1` |
| Hook prefix | `flexa_mailbridge.` (**dot-separated** — generation mới) |
| Option prefix | `flexa_mailbridge_` (option cấu hình duy nhất: `flexa_mailbridge_settings`) |
| JS global | `flexaMailBridge` (`window.flexaMailBridge`) |
| React mount id | `flexa-mailbridge-admin-root` |
| Tailwind prefix | `fs:` |
| Zustand persist key | `flexa-mailbridge:ui` |
...
- Hooks **dot-separated**: `flexa_mailbridge.settings.updated` (không dùng slash)
...
- **REST**: namespace qua `Api\Endpoint::NAMESPACE`; **mọi route có `permission_callback` thật** + per-arg `sanitize_callback`; trả `WP_REST_Response|WP_Error` kèm status; đăng ký **mọi** controller trong `Api\Router::register_routes()` (kết bằng `do_action('flexa_mailbridge.rest.register_routes')`). Controller không add vào Router = chết thầm.
- **Settings**: một option `flexa_mailbridge_settings`, schema/defaults/sanitizer trong `Support\Settings` (typed `defaults()`, type-coercing `all()/get()`, `sanitize()` drop key lạ). Update **merge-over-stored** (partial update), bắn `flexa_mailbridge.settings.updated` ($new,$old).
- **Permissions**: chỉ qua `Support\Capabilities` (`can_manage()`≈`manage_options` cho SMTP settings), mỗi cái bọc filter `flexa_mailbridge.capabilities.*`.
- **Destructive**: chỉ một `Maintenance\Eraser::erase_all()` dùng chung REST danger-zone + WP-CLI; bắn `flexa_mailbridge.data_reset`.
...
- i18n qua `@/lib/i18n` (text-domain `flexa-mailbridge`), **literal string** để makepot trích tĩnh.
- `cn()` từ `@/lib/cn`; mọi class Tailwind mang prefix `fs:`. shadcn hand-vendored trong `components/ui/`.
- Mọi mount bọc `<AppProviders>` (queryClient singleton + theme); root id `flexa-mailbridge-admin-root`.
...
```
flexa-mailbridge.php                 # bootstrap: define(), autoloader fallback, boot
...
├─ Support/
│  ├─ Settings.php             # option flexa_mailbridge_settings (schema/defaults/sanitize/merge)
│  ├─ Capabilities.php
...
|----|-----|----------|-----------|-----|
| **WP0** | Foundation | `flexa-mailbridge.php` bootstrap (define block, autoloader fallback), `Plugin::boot()`, `Concerns\HasInstance`, `Api\Endpoint` base + `Api\Router` rỗng, `Setup\Activator/Deactivator`, `Database\Schema` (khung DB_VERSION), `.gitignore`/`.distignore`/`release.sh`, `docs/CODE_STYLE_AND_UI.md`. | — | Plugin activate không lỗi, phpstan L6 sạch |
| **WP1** | Settings | `Support\Settings` (defaults typed, sanitize merge), `Support\Capabilities`, `Support\Encryption` (AES-256-GCM), `SettingsEndpoint` (GET/POST partial-merge), seed option lúc activate. | WP0 | Lưu/đọc settings qua REST, secret mã hoá, `flexa_mailbridge.settings.updated` bắn |
...
| **WP10** | CLI | `LogsCommand` (list filter status/date, format table/csv/json), `SendTestCommand`, `ResetCommand` (gọi `Eraser`). | WP5 | `wp flexa-mailbridge log list` chạy |
...
- `{prefix}flexa_mailbridge_email_logs` — id, subject, email_from, email_to (serialized), mailer, date_time, status (tinyint), content_type, body_content (longtext), reason_error, source, extra_info (json), flag_delete.
- `{prefix}flexa_mailbridge_open_events` — id, log_id (idx), count, date_time, extra_info.
- `{prefix}flexa_mailbridge_click_events` — id, log_id (idx), url, count, date_time, extra_info.

### 5.4 Hooks mở rộng (dot-separated)
`flexa_mailbridge.rest.register_routes` · `flexa_mailbridge.settings.updated` · `flexa_mailbridge.data_reset`
· `flexa_mailbridge.capabilities.*` · `flexa_mailbridge.settings.mailer_schema` · `flexa_mailbridge.mailers.register`
· `flexa_mailbridge.mail.before_send` · `flexa_mailbridge.mail.sent` · `flexa_mailbridge.mail.failed`
· `flexa_mailbridge.log.to_array` · `flexa_mailbridge.logs.purged`
· `flexa_mailbridge.pro.is_licensed`
...
6. Pro split kiểu Flexa (`Flexa\MailBridgePro\` → `src-pro/`, gate bằng file-absence + `flexa_mailbridge.pro.is_licensed`) — không ship code disabled.
