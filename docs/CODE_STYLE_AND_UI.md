# Flexa MailBridge — Code Style & UI Guide

> Derived from the canonical Flexa lineage plugins (**flexa-cache**,
> **flexa-media-folders-pro**). Authoritative style/UI spec for flexa-mailbridge; the
> phased build (`docs/IMPLEMENTATION_PLAN.md`) targets it. All examples use
> flexa-mailbridge's naming already substituted (§6). Copy patterns verbatim.

## 1. Lineage — new (de-branded) generation

flexa-mailbridge uses the **newer generation** of the Flexa scaffold (chosen 2026-08-30):
...
`Install\Migrator`). Hooks are **dot-separated** (`flexa_mailbridge.settings.updated`),
...
| Namespace | one class per file, PSR-4, `Flexa\MailBridge\` → `src/` (Pro: `Flexa\MailBridgePro\` → `src-pro/`) |
...
| Constants | `UPPER_SNAKE`; bootstrap `define()` block `FLEXA_MAILBRIDGE_*` (`VERSION`, `FILE`, `PATH`, `URL`, `BASENAME`, `REST_NAMESPACE`, `TEXT_DOMAIN`) |
| Hooks | dot-separated: `flexa_mailbridge.rest.register_routes`, `flexa_mailbridge.settings.updated`, `flexa_mailbridge.data_reset`, `flexa_mailbridge.capabilities.*`, `flexa_mailbridge.mail.before_send`, `flexa_mailbridge.mail.sent`, `flexa_mailbridge.mail.failed`, `flexa_mailbridge.pro.is_licensed` |
| Options | `flexa_mailbridge_*`; the ONE settings option is `flexa_mailbridge_settings` |
| REST | namespace `flexa-mailbridge/v1` via `Api\Endpoint::NAMESPACE`; **every route has `permission_callback`**; per-arg `sanitize_callback`; return `WP_REST_Response\|WP_Error` with explicit status; endpoints registered in `Api\Router::register_routes()` which ends with `do_action( 'flexa_mailbridge.rest.register_routes' )` |
| Permissions | via `Support\Capabilities` (`can_manage()` = `manage_options`, `can_manage_settings()` = `manage_options`), each wrapped in a `flexa_mailbridge.capabilities.*` filter |
| Secrets | provider API keys / SMTP passwords encrypted via `Support\Encryption` (AES-256-GCM, authenticated) before persisting into `flexa_mailbridge_settings` — never store plaintext |
...
| Destructive | one `Maintenance\Eraser::erase_all()` used by BOTH the REST danger-zone and WP-CLI; fires `flexa_mailbridge.data_reset` |
...
- i18n: `__`, `_x`, `_n`, `sprintf` from `@/lib/i18n` (text domain `flexa-mailbridge`), **string literals only**.
- `cn()` from `@/lib/cn`; every Tailwind class carries the `fs:` prefix.
- shadcn primitives hand-vendored in `components/ui/` (no CLI dep).
- Every mount wrapped in `<AppProviders>` (module-singleton `queryClient` + theme). Root id `flexa-mailbridge-admin-root`. JS global `window.flexaMailBridge`.

## 4. Mailer provider contract (WP2+)

Every provider implements `Flexa\MailBridge\Mailer\Contracts\MailerInterface`
...
slug/text-domain `flexa-mailbridge` · namespace `Flexa\MailBridge\` · const `FLEXA_MAILBRIDGE_` ·
REST `flexa-mailbridge/v1` · hook prefix `flexa_mailbridge.` · option `flexa_mailbridge_settings` ·
JS global `flexaMailBridge` · mount `flexa-mailbridge-admin-root` · tw prefix `fs:` ·
zustand key `flexa-mailbridge:ui`.
