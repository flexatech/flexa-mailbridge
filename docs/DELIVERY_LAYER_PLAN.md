# Flexa MailBridge → Email Delivery Layer — Plan (evolve, không rewrite)

> Kế hoạch tiến hóa `flexa-mailbridge` từ "cấu hình SMTP + gửi mail" thành **lớp
...
PSR-4 `Flexa\MailBridge\` → `src/`, singleton qua `Concerns\HasInstance`, boot tập
...
`flexa-mailbridge`. REST namespace `flexa-mailbridge/v1`, mọi route có `permission_callback`
...
wp_mail()
  └─ PhpMailerBridge (subclass PHPMailer cài vào global $phpmailer @ plugins_loaded:100)
       └─ MailerManager::dispatch( $php )
            ...
            ├─ do_action flexa_mailbridge.mail.before_send  ($php, $slug)
            ├─ attempt(): ConfiguresPhpMailer->configure + send_native()  |  API mailer->send(Message): Result
            ├─ ok  → do_action flexa_mailbridge.mail.sent   ($php, $slug, meta)
            └─ fail→ do_action flexa_mailbridge.mail.failed ($php, $slug, error, meta)  → throw → wp_mail_failed
```
...
`flexa_mailbridge.log.created($id,$php)` → settle SENT/FAILED lúc có kết quả. Snapshot
...
(`flexa_mailbridge.settings.mailer_schema`, `flexa_mailbridge.mailers.register`). **Đã
provider-agnostic** đúng yêu cầu.

### 1.5 DB tables / options
Bảng (`Database\Schema`, DB_VERSION `0.1.0`):
- `flexa_mailbridge_email_logs` — id, subject, email_from, email_to (serialized),
  ...
  `mailer`, `date_time`.
- `flexa_mailbridge_open_events`, `flexa_mailbridge_click_events` — log_id (idx), count, ...

Options: `flexa_mailbridge_settings` (1 option duy nhất, schema/defaults/sanitize/merge
trong `Support\Settings`, secret AES-256-GCM, 3 view raw/all/for_rest),
`flexa_mailbridge_db_version`, `flexa_mailbridge_oauth_tokens`,
`flexa_mailbridge_imported_log_sources`, `flexa_mailbridge_import_notice_dismissed`.
...
**Chỉ WP-Cron native.** `flexa_mailbridge_retention` (daily), `flexa_mailbridge_report_weekly`
(weekly), `flexa_mailbridge_report_monthly` (custom 30d). `Scheduler::sync()` reconcile
...
Thêm cột vào `flexa_mailbridge_email_logs` (dbDelta an toàn, dữ liệu cũ nhận default):
...
- **Nguyên tắc mạng:** check có I/O (DNS lookup, socket connect) **chỉ chạy trong
  cron `flexa_mailbridge_health` (daily) hoặc refresh admin** (`POST /health/run`).
  `GET /health` **chỉ đọc cache** (option `flexa_mailbridge_health_report`) → mở màn
...
- `HealthChecker`: `run()` (chạy + cache), `run_scheduled()` (cron void),
  ...
  seam `flexa_mailbridge.health.checks` cho plugin ngoài thêm check.
...
### Bảng queue (`flexa_mailbridge_email_queue`, migration 0.2.0 → 0.3.0)
...
- **Idempotency:** key = hash(from|recipients|subject|md5(body)). Dedupe best-effort:
  ...
  không trùng thì để retry OFF (hoặc dùng filter `flexa_mailbridge.queue.should_retry`).
...
Action: `flexa_mailbridge.queue.sent`, `flexa_mailbridge.queue.retry_scheduled`,
`flexa_mailbridge.queue.failed`. Filter: `flexa_mailbridge.queue.dedupe`,
`flexa_mailbridge.queue.backoff`, `flexa_mailbridge.queue.batch_size`,
`flexa_mailbridge.queue.time_budget`, `flexa_mailbridge.queue.should_retry`,
`flexa_mailbridge.queue.use_action_scheduler`, `flexa_mailbridge.mail.bypass_queue`.

---

## 6. Phạm vi (100% free — KHÔNG license/pro trong plugin này)
Plugin này là **hoàn toàn miễn phí**. WordPress.org cấm license-gating /
upsell-code trong plugin, nên **tuyệt đối không** có `flexa_mailbridge.pro.is_licensed`,
...
| DL2 Health + Monitoring | ☑ xong | **Health** (`src/Health/`): `CheckResult` VO (status PASS/WARN/ERROR/NOT_CHECKED/NA + group/id/label/message/remediation/context, context chỉ chứa dữ liệu không nhạy cảm) · `HealthReport` (overall = status tệ nhất; NOT_CHECKED/NA trung tính, không kéo tụt) · `Contracts\HealthCheck` (id/label/run → list CheckResult) · 3 check: `EnvironmentCheck` (mailer đang chọn có cấu hình?, from_email hợp lệ?, dev-mode, phát hiện plugin khác override `wp_mail()` qua ReflectionFunction → path rút gọn tương đối, không lộ layout server), `TransportCheck` (SMTP: fsockopen host:port timeout 5s, **không gửi credential**; API: kiểm creds có mặt, connectivity thật để "Send test email"; PHP mail() → N/A), `DnsCheck` (SPF/DMARC tra `dns_get_record` → advisory WARN không phải ERROR; **DKIM = NOT_CHECKED** vì selector provider-specific; domain private/localhost → N/A). `HealthChecker` service: chạy check **chỉ trong cron `flexa_mailbridge_health` (daily) hoặc refresh admin**, cache vào option `flexa_mailbridge_health_report`, `cached()` đọc option (an toàn mọi request, không I/O); seam filter `flexa_mailbridge.health.checks` cho Pro. **Monitoring** (`src/Monitoring/Monitor.php`): `snapshot(days)` tổng hợp trên log — per-provider sent/failed/failure_rate/avg_duration_ms/last_sent/last_failed + health signal (healthy/degraded/failing, dưới 5 mẫu → insufficient_data), category breakdown, `scope: local` đánh dấu rõ "metrics site bạn ≠ provider-wide". `EmailLogRepository` +`provider_stats()` +`category_counts()` (prepared, GROUP BY, dùng cột DL1 error_category/duration_ms). **REST**: `GET /health` (cache, không chạy check), `POST /health/run` (chạy + cache, settings cap), `GET /monitoring?days=N`; đăng ký trong Router. **Lifecycle**: `Plugin::boot` +HealthChecker; Deactivator +unschedule `flexa_mailbridge_health`; Eraser + uninstall.php +xoá option health. **Không đổi schema** (BC ~0). **Verify**: php -l sạch 16 file; **phpstan L6 sạch**; **phpcs sạch**; test overall-status 6 case + shape PASS; autoload PSR-4 resolve 10 class mới OK. Không remote call nào trong request frontend (check chỉ ở cron/refresh; GET đọc cache). |
